...
function computeSourceKey(src) {
    if (Array.isArray(src))
        return src.map(computeSourceKey).join('|');
    if (typeof src === 'number')
        return String(src); // require('./img.png')
    if (typeof src === 'string')
        return src; // remote on web
...
const WrappedImage = forwardRef(function WrappedImage(props, ref) {
    ...
    const handleError = useCallback((e) => {
        ...
        if (fallbackSource || Array.isArray(source))
            return;
    ...
    return (_jsx(ExpoImage.Image, { ...props, source: fallbackSource ?? source, ref: ref, onError: handleError }));
});
